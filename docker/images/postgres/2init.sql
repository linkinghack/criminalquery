CREATE TABLE public.b_users (
    ...
    ch_user_id varchar(64) NOT NULL,
...
COMMENT ON COLUMN public.b_criminals.ch_address
    IS '住所地址';
COMMENT ON COLUMN public.b_criminals.dt_created_at
    IS '逃犯信息入库时间';
COMMENT ON COLUMN public.b_criminals.dt_updated_at
    IS '逃犯个人信息更新日期时间';
...
COMMENT ON COLUMN public.b_criminal_contacts.d_birthday
    IS '联系人生日';
COMMENT ON COLUMN public.b_criminal_contacts.ch_realation
    IS '联系人与逃犯关系';
...
COMMENT ON COLUMN public.b_districts.i_supervisor_id
    IS '父级区划id';
COMMENT ON COLUMN public.b_districts.ch_path_root
...
CREATE TABLE public.b_clue (
    ...
    d_created_at timestamp with time zone NOT NULL,
    PRIMARY KEY (i_id)
);
...
COMMENT ON COLUMN public.b_clue.ch_description
    IS '线索描述';
COMMENT ON COLUMN public.b_clue.d_created_at
    IS '线索上传时间';
